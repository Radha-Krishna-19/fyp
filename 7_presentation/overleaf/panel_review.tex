...
Size-matched comparison & 4 synthesised MOFs expanded under the identical 48\,\AA{} rule
and compared against the 77 & \textcolor{okGreen}{\textbf{Functional}} \\
Dataset release & 81 frameworks, CC BY 4.0, with schema, provenance and the withdrawn
claim documented & \textcolor{okGreen}{\textbf{Functional}} \\
Experimental-set validation at scale & Provenance bug fixed; notebook ready; the
50-structure run needs a live query & \textcolor{warnRed}{\textbf{Planned}} \\
\bottomrule
\end{tabular}
\end{table}
\vspace{-0.32cm}
...
\textbf{Engineering $\approx$85\%}: 14 Python modules, an independent browser
implementation that cross-checks them, a 9-page platform, 5 CI-gating test suites, the
77-framework study end to end, a released dataset.
\textbf{Validation $\approx$60\%}: the descriptor is provably correct and its failure modes
are documented --- the size confound is identified, quantified and reported. Not established:
behaviour across chemical families, untestable on a single-metal set.
\textbf{Weighted $\approx$73\%} (rubric asks $\approx$60\%).
\textbf{Verification:} Python and JavaScript agree on every value ($\tau(0)=-2.6551$,
...
\textbf{Priority 1 --- 50 synthesised frameworks} \hfill \textcolor{warnRed}{\textbf{blocking}}
\begin{itemize}\scriptsize
  \item The provenance bug is \textbf{fixed}: records are now filtered on their own
        \texttt{database} field instead of trusting the server-side parameter.
  \item One run settles three questions at once --- cross-family separation, whether the
        real-MOF gap is real, and whether the band means anything beyond one generator.
  \item \textbf{Nothing needs to be built.} It is a data task.
\end{itemize}

\textbf{Priority 2 --- Confirm the size control at scale} \hfill \textcolor{okGreen}{\textbf{method proven}}
\begin{itemize}\scriptsize
  \item Size-matching already works: the 4 synthesised structures were expanded under the
        identical 48\,\AA{} rule before comparison.
  \item Repeat the partial-correlation check on the larger set.
\end{itemize}
...
\vspace{0.08cm}
\textbf{New:} four \textit{synthesised} MOFs, size-matched, fall clear of the hypothetical
band --- $\Delta\alpha=1.66$--$2.26$ vs $1.02$--$1.41$, gap $0.25$, no overlap. MOF-5 shares
the Zn$_4$O node and block count of much of the 77 and still separates: neither a size nor a
metal artefact. Four archetypes are not a sample --- 50 structures decide it.
\end{block}
\begin{block}{\footnotesize On reporting negative results}
\scriptsize
We report the size confound rather than the raw $-0.497$ correlation. A band built on an
uncontrolled size effect would not survive scrutiny; quantifying it is more defensible.
\end{block}
\end{columns}
\end{frame}
