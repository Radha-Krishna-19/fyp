...
\vspace{-0.32cm}
{\tiny
\textbf{How the $\approx$60\% is arrived at (not asserted).}
\textbf{Engineering $\approx$80\%}: 14 Python modules (2{,}681 lines), a cross-validated
browser implementation (16 JS modules), an 18-section platform, 8 automated test suites, a
77-framework study running end to end.
\textbf{Scientific validation $\approx$35\%}: the descriptor is computed correctly, but its
\textit{physical meaning} is not yet established --- the size confound is unresolved and the
dataset is hypothetical and single-metal.
\textbf{Weighted: $\approx$60\%.}
\ \ \textbf{Verification:} Python and JavaScript agree on every reported value
($\tau(0)=-2.6551$ on HKUST-1 in both).
Code: \textbf{github.com/Radha-Krishna-19/fyp}}
\end{frame}
